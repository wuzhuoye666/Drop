...
| B4 | 状态机PENDING→RUNNING→UPLOADING→DONE/FAILED，每次迁移落库带reason | 基础能力4 | apiserver/task.go(updateTaskStatus), drop/server/*Service | ⬜ 未开始 | 0% |
...
| B6 | 结构化日志、显式错误处理、单测≥50%、≥3端到端集成测试 | 基础能力6 | 全组件 | ⬜ 未开始 | 0% |
...
| E2 | eBPF采集器：使用libbpf/bcc/bpftrace，至少一个内核态探针 | 扩展-多采集器1 | drop/agent/EbpfProfiler | ⬜ 未开始 | 0% |
...
| E4 | 智能归因：火焰图+元数据+baseline结构化喂给LLM，LLM只能调用定义的工具，产出可验证结论 | 加分-智能归因 | analysis/analysis_advisor.py, analysis/hunyuanApi.py | ⬜ 未开始 | 0% |
